...
class DataBuffer:
    ...
    def _on_window_ready(self, window_rows: List[Tuple[float, ...]], window_ts: List[float]) -> None:
        ...
        # LEFT
        quatLW_x, quatLW_y, quatLW_z, quatLW_w = self._align_xyzw_raw(quatLW_x, quatLW_y, quatLW_z, quatLW_w, apply=False)

        # Skip first window entirely for warmup
        if not self._calibrated and self._windows_emitted == 1:
            log_system("f[DataBuffer] Skipping window #1 for data stabilization. Next window will calibrate")
            return
